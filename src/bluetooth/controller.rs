...
pub async fn controller_info() {
    ...
    output_vec.iter().for_each(|f| {
        let state = global.write().unwrap();
        if f.contains("Powered") {
            state.controller_info.write().unwrap().powered = match extract_value(f) {
                "yes" => true,
                "no" => false,
                _ => false,
            };
        } else if f.contains("PowerState") {
            state.controller_info.write().unwrap().power_state = extract_value(f).to_owned();
        } else if f.contains("Pairable") {
            state.controller_info.write().unwrap().pairable = match extract_value(f) {
                "yes" => true,
                "no" => false,
                _ => false,
            }
        } else if f.contains("Discoverable") {
            state.controller_info.write().unwrap().discoverable = match extract_value(f) {
                "yes" => true,
                "no" => false,
                _ => false,
            }
        } else if f.contains("Name") {
            state.controller_info.write().unwrap().name = Some(extract_value(f).to_owned());
        }
    });
}
